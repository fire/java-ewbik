...
public class IKTest_SinglePrecision extends PApplet {
    ...
    public void draw() {
        ...
        if (mousePressed) {
            ...
            simpleArmature.IKSolver(
                    ...
                    0.1f, // dampening parameter (in radians), determines the maximum amount a bone can
                    // rotate per iteration step.
                    // keeping this value low will result in higher quality poses, but those poses
                    // will take more iterations to converge.
                    10, // inverse weighting helps prevent bones near the target from doing all of the
                    // work
                    // so you don't need to rely on small dampening factors mixed with high
                    // iteration counts
                    // to get natural looking poses, but it makes armatures with multiple end
                    // effectors more unstable.
    ...
    public void drawBoneInfo(Bone bone) {
        ...
        try {
            ...
            boneAngles += "  ( " + degrees((float) angleArr[0]) + ",   " + degrees((float) angleArr[1]) + ",   "
                    + degrees((float) angleArr[2]) + "  )";
            fill(0);
    ...
    public void keyPressed() {
        if (key == CODED) {
            if (keyCode == UP) {
                int currentPinIndex = min(pinnedBones.size() - 1,
                        max((pinnedBones.indexOf(activePin) + 1) % pinnedBones.size(), 0));
