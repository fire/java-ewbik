...
public class Vector3 implements CanLoad {
    ...
    public Vector3 getPlaneProjectionOf(Vector3 p1, Vector3 p2) {
        return this.getPlaneProjectionOf((Vector3) p1.crossCopy(p2));
    }
    ...
    public Vector3 getPlaneProjectionOf(Vector3 rawNorm) {
        Vector3 norm = (Vector3) rawNorm.copy().normalize();
        Vector3 normProj = (Vector3) norm.multCopy(this.dot(norm));
        normProj.multiply(-1);

        return (Vector3) normProj.addCopy(this);
    }
     ...
     * which is in the range [0,1]. The result is stored
     * in this vector.
     *
    ...
    public Vector3 setMagSq(float len2) {
        float oldLen2 = magSq();
        return (oldLen2 == 0 || oldLen2 == len2) ? (Vector3) this
    ...
    /**
     * makes a copy of this vector and multiplies it componentWise by the given
     * vector, then returns that copy.
     *
     * @param v The vector
     * @return the resulting vector
     */
